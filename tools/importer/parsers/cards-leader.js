...
export default function parse(element, { document }) {
  ...
  // Collect leader cards from the document, excluding header/footer experience fragments
  // (header mega-menu contains .cardpagestory.card-dashboard cards that must be skipped)
  const allCards = [...document.querySelectorAll('.cardpagestory')].filter(
    (card) => !card.closest('.cmp-experiencefragment--header') && !card.closest('.cmp-experiencefragment--footer'),
  );
