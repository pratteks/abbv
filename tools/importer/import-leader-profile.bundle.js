var CustomImportScript = (() => {
  ...
  function parse(element, { document }) {
    ...
    if (gridRow) {
      ...
      if (contentCols.length > 0) {
        ...
        const breadcrumbBlock2 = buildBreadcrumbBlock(element, document);
        if (breadcrumbBlock2) wrapper.appendChild(breadcrumbBlock2);
  ...
  function transform(hookName, element, payload) {
    if (hookName === TransformHook.beforeTransform) {
      ...
      element.querySelectorAll(".container.overlap-predecessor").forEach((container) => {
        const bc = container.querySelector(".cmp-breadcrumb, .abbvie-breadcrumb, .breadcrumb");
        if (bc) {
          const firstLink = bc.querySelector("a[href]");
          if (firstLink) {
            container.setAttribute("data-breadcrumb-home", firstLink.getAttribute("href"));
            const firstText = firstLink.textContent.trim();
            if (firstText) container.setAttribute("data-breadcrumb-home-title", firstText);
          }
        }
      });
      WebImporter.DOMUtils.remove(element, [
      ...
      element.querySelectorAll('img[src*="scene7.com"]').forEach((img) => {
        ...
        if (src) {
          ...
          if (qIdx !== -1) img.setAttribute("src", src.substring(0, qIdx));
        }
      });
    }
