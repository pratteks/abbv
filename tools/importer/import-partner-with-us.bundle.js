var CustomImportScript = (() => {
  ...
  // tools/importer/import-partner-with-us.js
  var import_partner_with_us_exports = {};
  __export(import_partner_with_us_exports, {
    default: () => import_partner_with_us_default
  });
  ...
  function parse3(element, { document }) {
    ...
    const buttonContainer = ctaButton.closest(".button");
    const isExternal = buttonContainer && buttonContainer.classList.contains("external-cta");
    const isInternal = buttonContainer && buttonContainer.classList.contains("internal-cta");
    const isBack = buttonContainer && buttonContainer.classList.contains("back-cta");
    let ctaType = "default-cta";
    if (isExternal) ctaType = "external-cta";
    else if (isInternal) ctaType = "internal-cta";
    else if (isBack) ctaType = "back-cta";
    const ariaLabel = ctaButton.getAttribute("aria-label") || "";
    const frag = document.createDocumentFragment();
    const children = element.querySelectorAll(":scope > div");
    let ctaBlockInserted = false;
    children.forEach((child) => {
      ...
      if (cls.includes("image") && !cls.includes("abbvie-logo")) {
        const img = child.querySelector(".cmp-image__image") || child.querySelector("img");
        if (img) {
          ...
          pImg.appendChild(imgEl);
          frag.appendChild(document.createComment(" field:image "));
          frag.appendChild(pImg);
        }
        return;
      }
      if (cls.includes("button") && !ctaBlockInserted) {
        const cells = [];
        ...
        frag.appendChild(block);
        ctaBlockInserted = true;
        return;
      }
      if (cls.includes("linklist") || cls.includes("list")) {
        const listLinks = child.querySelectorAll(".cmp-list__item-link");
        listLinks.forEach((listLink) => {
          const href = listLink.getAttribute("href") || "";
          const titleSpan = listLink.querySelector(".cmp-list__item-title");
          const linkText = titleSpan ? titleSpan.textContent.trim() : listLink.textContent.trim();
          if (linkText && href) {
            const pLink = document.createElement("p");
            const aLink = document.createElement("a");
            aLink.setAttribute("href", href);
            aLink.textContent = linkText;
            pLink.appendChild(aLink);
            frag.appendChild(pLink);
          }
        });
        return;
      }
  ...
  var import_partner_with_us_default = {
    transform: (payload) => {
  ...
  return __toCommonJS(import_partner_with_us_exports);
})();
