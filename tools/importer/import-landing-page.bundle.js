var CustomImportScript = (() => {
  ...
  function parse(element, { document }) {
    ...
    const bgPicture = prevSibling?.querySelector("picture") || prevSibling?.querySelector(".cmp-container__bg-image") || prevSibling?.querySelector("img");
    ...
    contentCols.forEach((col) => {
      col.querySelectorAll("img").forEach((img) => {
        if (!img.alt || img.alt === "") {
          ...
          const ariaLabel = img.getAttribute("aria-label") || img.closest("[aria-label]")?.getAttribute("aria-label");
          if (ariaLabel) {
            img.alt = ariaLabel;
          }
        }
