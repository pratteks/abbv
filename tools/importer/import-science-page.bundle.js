var CustomImportScript = (() => {
  ...
  function transform(hookName, element, payload) {
    if (hookName === H.before) {
      ...
      const listNone = element.querySelectorAll(".list-results-none");
      console.log(`[cleanup] Found ${listNone.length} .list-results-none elements`);
      listNone.forEach((el) => el.remove());
      ...
      allElements.forEach((el) => {
        ...
        if (/^No results found/.test(txt) || /^Change your search criteria/.test(txt)) {
          console.log(`[cleanup] REMOVING <${el.tagName.toLowerCase()}> "${txt.substring(0, 60)}" class="${el.className}"`);
          el.remove();
          removedCount++;
        }
      });
      console.log(`[cleanup] Removed ${removedCount} "No results found" elements`);
  ...
  function consolidateSectionMetadata(root) {
    ...
    children.forEach((child) => {
      if (child.tagName === "HR") {
        ...
        const name = child.rows && child.rows[0] && child.rows[0].cells && child.rows[0].cells[0] && child.rows[0].cells[0].textContent.trim().toLowerCase();
