...
function buildInput(field) {
  ...
  input.name = input.id || label;
  if (maxlength) input.dataset.maxlength = maxlength;
...
/**
 * Handles form submission for standard forms
 ...
 * @param {Object} field - Field configuration object
 * @param {FormTypeHandler} [formTypeHandler] - Form type handler for special fields
 * @returns {HTMLElement} Form field element (fieldset, div, or button)
 */
async function buildField(field, formTypeHandler = null) {
  const {
    type,
    label,
    help,
  ...
  // Check if form type handler can create this field
  if (formTypeHandler) {
    const specialField = await formTypeHandler.createSpecialField(field);
    if (specialField) {
      return specialField;
    }
  }

  // Handle common special categories
  if (category === 'Filter_text') {
    if (type === 'heading') {
  ...
  } else if (category === 'Terms and Condition') {
    const termsDiv = document.createElement('div');
    termsDiv.className = 'terms-checkbox';
    const termsText = processTextWithLinksAndNewlines(label);
    termsDiv.innerHTML = `<input type="checkbox" id="terms" name="terms" required/>
            <label for="terms">${termsText}</label>
            <p
              class="field-error"
              style="display:none; color:#e74c3c; font-size:14px; margin-top:5px;"
            >${errorMessage}</p>`;
    return termsDiv;
  } else if (category === 'recaptcha_text') {
    const recaptchaEle = document.createElement('p');
    recaptchaEle.className = 'recaptcha-text';
    recaptchaEle.innerHTML = processTextWithLinksAndNewlines(label);
    return recaptchaEle;
  }
  ...
  if (type === 'button' && category === 'Submit') {
    return `<button type="submit" class="submit-btn" data-error="${
      errorMessage || ''
    }">${field.Label || field.Type || 'Submit'}</button>`;
  }
  ...
  if (errorMessage) {
    ...
    wrapper.appendChild(errorText, input);
  }
 ...
 * @param {string} [submit] - Submit URL
 * @param {FormTypeHandler} [formTypeHandler] - Form type handler for special fields
 * @returns {Promise<HTMLFormElement>} Complete form element
 */
async function buildForm(fields, submit, formTypeHandler = null) {
  ...
  inputElements.className = 'inputs-container';
  const filterElements = document.createElement('div');
  filterElements.className = 'filters-container';
  const tncElements = document.createElement('div');
  tncElements.className = 'terms-conditions';
  const submitButton = document.createElement('div');
  submitButton.className = 'submit-wrapper';

  // Separate fields into buttons and non-buttons
  const buttonFields = fields.filter((field) => field.type === 'submit' || field.type === 'reset');
  ...
  buttons.push(...buttonFields);

  const fieldElements = await Promise.all(
    nonButtonFields.map(async (field) => ({
      field,
      element: await buildField(field, formTypeHandler),
    })),
  );

  // Process each field element according to its category
  fieldElements.forEach(({ field, element: fieldElement }) => {
    if (field?.category === 'Terms and Condition') {
      tncElements.innerHTML += fieldElement.outerHTML;
    } else if (field?.category === 'Filter_text') {
      filterElements.innerHTML += fieldElement.outerHTML;
    } else if (field?.category === 'Filters') {
      filterElements.innerHTML += fieldElement.outerHTML;
    } else if (field?.category === 'Input') {
      fieldElement.classList.add('input-wrapper');
      inputElements.innerHTML += fieldElement.outerHTML;
    } else if (field?.category === 'Submit') {
      submitButton.innerHTML += fieldElement;
    } else if (field?.category === 'recaptcha_text') {
      submitButton.innerHTML += fieldElement.outerHTML;
  ...
  if (form.children.length === 0) {
    form.append(inputElements);
    form.append(filterElements);
    form.append(tncElements);
  ...
  if (buttons.length) {
    ...
    buttons.forEach(async (button) => buttonWrapper.append(
      await buildField(button, formTypeHandler),
    ));
      ...
      (entries) => {
        entries.forEach(async (entry) => {
          if (entry.isIntersecting) {
            try {
              // Initialize form type handler
              const formTypeHandler = new FormTypeHandler(block);
              await formTypeHandler.initialize();

              // Load form data
              const resp = await loadSpreadsheet(source);
              if (!resp.ok) { throw new Error(`${resp.status}: ${resp.statusText}`); }
              const { data } = await resp.json();
              if (!data) throw new Error(`No form fields at ${source}`);

              // Build form with type handler
              const form = await buildForm(data, submit, formTypeHandler);
              block.replaceChildren(form);

              // Finalize form with type-specific setup
              await formTypeHandler.finalizeForm(form);
