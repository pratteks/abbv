  ...
  if (status !== "loading" && status !== "loaded") {
    ...
    try {
      ...
      const decorationComplete = new Promise((resolve) => {
        (async () => {
          try {
            const mod = await loadBlockModule(blockName);
            if (mod.default) {
...
function mergeConfig(globalConfig, brandConfig) {
  ...
  mergedConfig.variations = [
    ...(globalConfig.variations || []),
    ...(brandConfig.variations || []),
  ];
  mergedConfig.cacheResetHandlers = [
    ...(globalConfig.cacheResetHandlers || []),
    ...(brandConfig.cacheResetHandlers || []),
  ];
  mergedConfig.decorations = {
    ...globalConfig.decorations,
    ...brandConfig.decorations,
  };
  return mergedConfig;
}
...
export async function loadBlockConfig(blockName) {
  ...
  return mergeConfig(
    await globalConfig?.default(),
    await brandConfig?.default(),
  );
}
...
export async function renderBlock(ctx) {
  ...
  if (blockConfig?.cacheResetHandlers) {
    window.cacheResetHandlers = [
      ...(window.cacheResetHandlers || []),
      ...blockConfig.cacheResetHandlers.map(
        (method) => () => method(ctx, blockConfig),
      ),
    ];
  }
